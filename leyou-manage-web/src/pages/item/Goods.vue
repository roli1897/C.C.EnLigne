<template>
  <v-card>
    <v-toolbar class="elevation-0">
      <v-btn color="primary" @click="addGoods">Nouveau produit</v-btn>
      <v-spacer/>
      <v-flex xs3>
        Statut：
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" color="pink" style="font-size: 12px; font-weight: bold;color: white" >Dépu</v-btn>
          <v-btn icon v-else>Publier</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
      ...
      <v-card>
        ...
        <v-card-actions class="elevation-10">
          <v-flex class="xs3 mx-auto">
            <v-btn @click="previous" color="primary" :disabled="step === 1">Étape précédente</v-btn>
            <v-btn @click="next" color="primary" :disabled="step === 4">Étape suivante</v-btn>
          </v-flex>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    data() {
      return {
        ...
        headers: [
          ...
          {text: 'Titre', align: 'center', sortable: false, value: 'title'},
          {text: 'Catégorie de produit', align: 'center', sortable: false, value: 'cname'},
          {text: 'Marque', align: 'center', value: 'bname', sortable: false,},
          {text: 'Opération', align: 'center', sortable: false}
        ],
        show: false,// 控制对话框的显示
        oldGoods: {}, // 即将被编辑的商品信息
        isEdit: false, // 是否是编辑
